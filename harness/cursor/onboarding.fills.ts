...
- **Git repository root + workspace trust**: Cursor discovers project hooks only from the Git root and runs them only in a trusted workspace. Initialize the project with \`git init\`, install AI-DLC at that exact root, mark it trusted in Cursor, and fully restart after changing repository or trust state.
